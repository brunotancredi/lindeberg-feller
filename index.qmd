...
">
  <figure style="margin:0; text-align:center;">
    ...
    <figcaption style="margin-top:4px; font-size:14px;">
...
I’m going to present generalizations of the Central Limit Theorem, such as the Lindeberg–Lévy–Feller Central Limit Theorem and Lyapunov’s Central Limit Theorem. 

The applications of the asymptotic behavior of the sample mean of random variables extend across all areas of statistics, being used for inference, hypothesis testing, estimation theory, among many others.
...
All the code is available in my [GitHub repository](https://github.com/brunotancredi/lindeberg-feller).
...
Let $\{X_n\}_{n \geq  1}$ be a sequence of **independent** and **identical distributed** random variables with finite expectation $\mu$ and positive, finite variance $\sigma^2$, and set $S_n = X_1 + X_2 + \dots + X_n, n \geq 1$. Then
...
But is easy to see that is equivalent to the one stated above
...
The issue with the theorem is that it relies on two strong assumptions: **independence** and **identical distribution**. But what if I told you we could relax one of them?
...
$$
L_2(n) = \max_{1 \leq k \leq n} \frac{\sigma^2_k}{V_n^2} \underset{n \rightarrow \infty}{\rightarrow} 0,
$$ {#eq-Lind2}


$L_2$: To understand condition $L_2$, it is helpful to consider situations in which it fails. 
The condition is violated when a single variance $\sigma_k^2$ constitutes a non-negligible fraction of the total variance $V_n^2$. 
For example, this can occur if

$$
\sigma_k^2 = \sum_{i=1}^{k-1} \sigma_i^2,
$$

since in that case the $k$th term alone contributes half of the total variance.

Thus, condition $L_2$ requires that no individual $X_k$ have variance that is large relative to the total variance; each term's contribution must become negligible as $n$ grows.
...
(ii). If $L_2$ (@eq-Lind2) and the convergence in distribution to a standard normal (@eq-LindCLT) both hold, then $L_1$ @eq-Lind1 holds as well.

:::

As we can see, we no longer require that the sequence be **identically distributed** to obtain a normal distribution. We are only requiring that $L_1$ is satisfied (i).

## What do we need to prove it?

The proof of the theorem is extensive, so here I’m going to present only a sketch of the proof.
...
Well, under certain assumptions we can use the Lindeberg-Lévy-Feller theorem to show that the OLS estimator asymptotically follows a normal distribution. For simplicity we are going to prove it for the estimator of the slope coefficient in a simple linear regression model (SLR).
...
Note that proving these we are proving $\hat{\beta}_{1n} \overset{d}{\rightarrow} \mathcal{N}(\beta_1, \frac{\sigma^2}{\sum_{i=1}^n(x_i - \bar{X}_n)^2})$ since

\begin{align*}
\hat{\beta}_{1n} &= \beta_1 + \frac{\sum_{i=1}^nW_{ni}}{\sum_{i=1}^n(x_i - \bar{X}_n)^2} \\
&= \beta_1 + \frac{\sigma}{\sqrt{\sum_{i=1}^n(x_i - \bar{X}_n)^2}} \cdot \frac{\sum_{i=1}^{n}W_{ni}}{\sigma\sqrt(\sum_{i=1}^{n}(x_i - \bar{X})^2)}\\
&\overset{d}{\rightarrow} \beta_1 + \frac{\sigma}{\sqrt{\sum_{i=1}^n(x_i - \bar{X}_n)^2}} \cdot Z = \mathcal{N}\left(\beta_1, \frac{\sigma^2}{\sum_{i=1}^n(x_i - \bar{X}_n)^2}\right)
\end{align*}
...
$$
\frac{1}{V_n^2} \sum_{i=1}^n (x_i - \bar X_n)^2 
...
$$
\le \frac{1}{V_n^2} \sum_{i=1}^n (x_i - \bar X_n)^2 
\mathbb{E}\!\left[
...
$$
= \frac{1}{V_n^2} \left[ \sum_{i=1}^n (x_i - \bar X_n)^2 \right]
\mathbb{E}\!\left[
e_1^2 \mathbf{1}\!\left\{ |e_1| \ge \frac{\varepsilon V_n}{M} \right\}
\right].
$$

Using the definition of $V_n^2  = \sigma^2 \sum_{i=1}^n (x_i - \bar X_n)^2$,

$$
= \frac{1}{\sigma^2} 
\mathbb{E}\!\left[
e_1^2 \mathbf{1}\!\left\{ |e_1| \ge 
\varepsilon \sigma \frac{\sqrt{\sum (x_i - \bar X_n)^2}}{M}
\right\} \right].
$$
...
\mathbb{E}\!\left[
e_1^2 \mathbf{1}\!\left\{ |e_1| \ge 
\varepsilon \sigma  \sqrt{\frac{\sum (x_i - \bar X_n)^2}{M^2}}
\right\}
\right].
$$
...
In @nte-proof, we conclude that we can change the normal distribution requirement for 
...
Sometimes, when working with a model whose assumptions require normality, we reassure ourselves with the argument: “I have enough data—the Central Limit Theorem guarantees that the distribution is approximately normal.” However, this is **not always true**.
...
In @fig-lindeberg_convergence, we can see that $L_1$ is not satisfied, since it converges extremely slowly to $0$ and does not reach it even at $n = 500{,}000$.
...
As a final comment, here we are working with citations in papers that have a certain limitation (the most cited papers here have around 50,000 citations). Now think about views of a webpage: how many views does [www.youtube.com](www.youtube.com) have compared to this humble page that talks about the Lindeberg–Lévy–Feller theorem? The variance there will be much higher, and this is just another example. So be careful, when your model assumes normality,because **a heavy-tailed distribution might be waiting for you around the corner**.

## Comic Relief

> The Swedish mathematician Harald Cramér met Lindeberg in 1922. He later recalled this story about Lindeberg and the beautiful farm he owned. “When he (Lindeberg) was reproached for not being sufficiently active in his scientific work, he said *‘Well, I am really a farmer.’* And if somebody happened to say that his farm was not properly cultivated, his answer was *‘Of course my real job is to be a professor.’* …” [@lindeberg_wikipedia]
